--[[
Scroll.lua
@Date    : 2016/9/15 下午4:05:29
@Author  : DengSir (tdaddon@163.com)
@Link    : https://dengsir.github.io
]]

local MAJOR, MINOR = 'Scroll', 1
...
function View:SetOffset(offset)
    self._offset = floor(offset + 0.5)
    if self.ScrollBar then
        self.ScrollBar:SetValue(self._offset)
    end
    self:Refresh()
end
...
function View:EnableHoldBottom(flag)
    self._holdBottom = flag or nil
end

function View:EnableHoldTop(flag)
    self._holdTop = flag or nil
end

function View:IsHoldBottom()
    return self._holdBottom
end

function View:IsHoldTop()
    return self._holdTop
end
